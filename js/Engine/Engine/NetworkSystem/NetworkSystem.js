...
var UNetworkSystem = /** @class */ (function (_super) {
    ...
    function UNetworkSystem() {
         ...
         * 对外接口
         ************************************************/
        // static Ins: UNetworkSystem = new UNetworkSystem();
        //网络连接状态
        _this.conState = ConnectionStatus.NO;
        _this.delay = 0;
        //1.回调注册表
        //命令编号、回调函数 调用者
        //一个接口一个函数||一个命令一个函数，注意不是多个函数
    ...
    //  先添加到容器，调用SendAll发送
    //  例子：一帧里面，把消息添加到队列
    UNetworkSystem.prototype.send = function (obj) {
        //TODO 处理成二进制
        //处理成JSON
        var str = JSON.stringify(obj);
    ...
    UNetworkSystem.prototype.sendAll = function () {
        var _this = this;
        this.sendBuffer.forEach(function (str) {
            _this.ws.send(str);
        });
        this.sendBuffer = [];
    };
